...
CComboListCtrl::CComboListCtrl()
{
	...
	m_dwDropDownCtrlStyle = WS_BORDER | WS_CHILD | WS_VISIBLE | WS_VSCROLL | ES_AUTOHSCROLL | ES_AUTOVSCROLL | 
...
BEGIN_MESSAGE_MAP(CComboListCtrl, CDragDropListCtrl)
...
void CComboListCtrl::OnHScroll(UINT iSBCode, UINT iPos, CScrollBar* pScrollBar) 
{
	...
	CDragDropListCtrl::OnHScroll(iSBCode, iPos, pScrollBar);
}
...
void CComboListCtrl::OnVScroll(UINT iSBCode, UINT iPos, CScrollBar* pScrollBar) 
{
	...
	CDragDropListCtrl::OnVScroll(iSBCode, iPos, pScrollBar);
}
...
void CComboListCtrl::OnLButtonDown(UINT iFlags, CPoint obPoint) 
{
	...
	CDragDropListCtrl::OnLButtonDown(iFlags, obPoint);
		...
		if ((GetItemState(iRowIndex, flag ) & flag) == flag)
		{
			...
			if (GetWindowLong(m_hWnd, GWL_STYLE) & LVS_EDITLABELS)
			{
				...
				if (IsCombo(iColumnIndex))
				{
					...
					GetParent()->SendMessage(WM_SET_ITEMS, (WPARAM)MAKELONG(iRowIndex,iColumnIndex), (LPARAM)&obComboItemsList);  
				...
				else if (!IsReadOnly(iColumnIndex))
				{
					...
					pInPlaceEdit->SetLimitText(m_nLimitTextCount);
				}
			}
		}
	}  
...
void CComboListCtrl::SetLimitTextCount(UINT nLimitTextCount/* =0 */)
{
	m_nLimitTextCount=nLimitTextCount; 
}
